...
class Echo:
    ...
    def system_prompt(self):
        return {
            "role": "system",
            "content": (
                "You are Atlas, a highly advanced lab assistant inspired by the characters TARS and CASE from the Interstellar films. "
        ...
        while True:
            try:
                ...
                message = self.session.prompt(
                    "User > ",
                    bottom_toolbar="You are speaking to Atlas. Ctrl-C to exit.",
                )
                ...
                print_formatted_text("Atlas > " + completion.choices[0].message.content)
